...
import OtherTagCourses from "./OtherTagCourses";
...
class ProfileItem extends React.Component {
	...
	addProjectInput = () => {
		const existingProjects = this.state.allProjects.map((fields) => ({
			...fields,
		}));
	...
	removeProjectInput = (targetIndex) => {
		this.setState({
			allProjects: this.state.allProjects.filter(
	...
	removeExpInput = (targetIndex) => {
		this.setState({
			allExp: this.state.allExp.filter(
				(item, index) => index !== targetIndex
			),
		});
	};
		...
		if (Object.keys(data).length > 1) {
			...
			if (update == 201) {
				this.setState({
					...
					gender:
						this.state.gender[0].toUpperCase() +
						this.state.gender.substring(
							1,
	...
	handleUpdateClick3 = async () => {
		...
		if (this.state.allProjects.length !== 0) {
			data.projects = this.state.allProjects.map(
				(fields) => ({ ...fields }["value"])
			); //projects
		}
		const update = await API.updateUserInfo(data);
	...
	handleUpdateClick4 = async () => {
		...
		if (this.state.allExp.length !== 0) {
			data.experience = this.state.allExp.map(
				(fields) => ({ ...fields }["value"])
			); //experience
		}
		const update = await API.updateUserInfo(data);
						...
						{this.state.isEditable1 ? (
							<View style={{ right: FULL_WIDTH * -0.03 }}>
								<TouchableOpacity
									...
									onPress={this.handleUpdateClick1.bind(this)}
								>
							...
							<View style={{ right: FULL_WIDTH * 0.05 }}>
								<TouchableOpacity
									style={styles.profileButtons}
									onPress={this.handleEditClick1.bind(this)}
								></TouchableOpacity>
							</View>
						)}
					</View>
				...
				<View style={styles.containerProfileItem2}>
					<View style={styles.profileCardHeader}>
						...
						{this.state.isEditable2 ? (
							<TouchableOpacity
								style={styles.profileButtons2}
								onPress={this.handleUpdateClick2.bind(this)}
							>
								<Check width={20} height={20} />
							</TouchableOpacity>
						) : (
							<TouchableOpacity
								style={styles.profileButtons2}
								onPress={this.handleEditClick2.bind(this)}
							></TouchableOpacity>
						)}
					</View>
					<View style={styles.info}>
					...
					<View>
						...
						<OtherTagCourses
							...
							courseChange={this.handleCourseChange.bind(this)}
							type="course"
						/>
					</View>
				...
				<View style={styles.containerProfileItem2}>
					<View style={styles.profileCardHeader}>
						...
						{this.state.isEditable3 ? (
							<TouchableOpacity
								style={styles.profileButtons3}
								onPress={this.handleUpdateClick3}
							></TouchableOpacity>
						) : (
							<TouchableOpacity
								style={styles.profileButtons3}
								onPress={this.handleEditClick3}
							></TouchableOpacity>
						)}
					</View>
						...
						return (
							<View key={index}>
								<TextInput
									...
									value={field.value}
									onChangeText={(text) =>
										this.onProjectTextChange(text, index)
									}
				...
				<View style={styles.containerProfileItem2}>
					<View style={styles.profileCardHeader}>
						...
						{this.state.isEditable4 ? (
							<TouchableOpacity
								style={styles.profileButtons4}
								onPress={this.handleUpdateClick4}
							>
								<Check width={20} height={20} />
							</TouchableOpacity>
						) : (
						...
						return (
							<View key={index}>
								<TextInput
									...
									value={field.value}
									onChangeText={(text) =>
										this.onExpTextChange(text, index)
									}
								...
								{this.state.isEditable4 ? (
									<TouchableOpacity
										disabled={!this.state.isEditable4}
										onPress={() =>
											this.state.isEditable4
												? this.addExpInput()
												: null
								...
								{this.state.isEditable4 ? (
									<TouchableOpacity
										...
										onPress={() =>
											this.state.isEditable4
												? this.state.allExp.length > 1
													? this.removeExpInput(index)
													: null
												: null
										}
										style={styles.cross}
