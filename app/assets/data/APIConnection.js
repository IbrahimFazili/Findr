...
class APIConnection {
	...
	async updateProfilePicture(email, type, checksum) {
		return (
			await fetch(
				`${this.ENDPOINT}:${this.PORT}/user/${email}/updateProfilePicture?type=${type}&checksum=${checksum}`
			)
		).text();
	}
	...
	async rightSwipe(src, target) {
		const swipeResult = await fetch(
			`${this.ENDPOINT}:${this.PORT}/rightSwipe?src=${src}&target=${target}`
		);
	...
	async leftSwipe(src, target) {
		const swipeResult = await fetch(
			`${this.ENDPOINT}:${this.PORT}/leftSwipe?src=${src}&target=${target}`
		);
	...
	async fetchPendingMatches(email) {
		return (
			await fetch(
				`${this.ENDPOINT}:${this.PORT}/user/${email}/pendingMatches`
			)
		).json();
	}
	...
	async fetchChatData(from, to) {
		return (
			await fetch(
				`${this.ENDPOINT}:${
					this.PORT
				}/fetchChatData?from=${from}&to=${to}&skipCount=${0}`
			)
		).json();
	}
